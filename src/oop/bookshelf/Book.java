...
public class Book {
    ...
    public static boolean isTitleWord(String word) {
        ...
        String[] prepositions = {"at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};

        if (Arrays.asList(articles).contains(lowercaseWord) ||
                Arrays.asList(conjunctions).contains(lowercaseWord) ||
                Arrays.asList(prepositions).contains(lowercaseWord)) {
            return false;
        }
    ...
    public void setTitle(String title) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if(i == 0 || isTitleWord(currentWord)) {
